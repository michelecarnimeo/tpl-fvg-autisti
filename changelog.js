...
const changelogData = [
  {
    ...
    time: '11:48',
    title: 'Modalità Test Globale + GPS Debug Panel + Fix vari (v1.8.7)',
    hidden: false,
    changes: [
      '🧪 MODALITÀ TEST GLOBALE:',
      '  • Sistema centralizzato per attivare funzionalità debug su tutte le pagine',
      '  • Toggle da pagina test.html con badge visivo persistente (🧪 TEST MODE)',
      '  • Stato salvato in localStorage, rimane attivo anche cambiando pagina',
      '  • Mostra/nasconde elementi debug (.debug-only) automaticamente',
      '📍 GPS DEBUG PANEL:',
      '  • Pannello floating draggable per visualizzare info GPS su mobile',
      '  • Mostra latitudine, longitudine, precisione, città e fermata più vicina',
      '  • Minimizzabile, spostabile, posizione salvata automaticamente',
      '  • Reverse geocoding con Nominatim per mostrare città della localizzazione',
      '  • Si apre automaticamente quando si preme "Rileva" in modalità test',
      '🔧 FIX GEOLOCALIZZAZIONE:',
      '  • Risolto errore "Cannot read properties of undefined (reading latitude)"',
      '  • Corretto accesso a position.latitude invece di position.coords.latitude',
      '  • Aggiunti controlli di validità posizione con log dettagliati',
      '🎨 FIX CSS E PERFORMANCE:',
      '  • Aggiunto -webkit-backdrop-filter per supporto Safari 9+',
      '  • Aggiunto -webkit-user-select e -moz-user-select per compatibilità completa',
      '  • Ottimizzata animazione badgePulse: usa pseudo-elemento invece di box-shadow',
      '  • Ottimizzata animazione subtle-highlight: usa opacity su pseudo-elemento',
      '  • Sostituiti CDN unpkg.com con file locali Leaflet (risolve header Expires)',
      '📄 FIX PAGINA TEST:',
      '  • Risolto problema scroll: aggiunto override per .test-page',
      '  • Spostato Debug Logger in sezione separata sopra Test Modalità PWA',
      '  • Aggiunto supporto toggle per gruppo debug (fix accordion)',
      '🌐 FIX CONNECTION MONITOR:',
      '  • Sostituito ping a Google con file locale ping.txt',
      '  • Risolve warning cache busting e migliora affidabilità',
      '✅ QUALITÀ:',
      '  • Tutti i DevTool warnings risolti',
      '  • Documentazione completa in MODALITA_TEST_GUIDA.md',
